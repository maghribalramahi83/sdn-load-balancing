...
class iplb(object):
    ...
    def _pick_server(self, key, inport):
        selected_server = random.choice(list(self.live_servers.keys()))
        total_connections = self.total_connection.get(selected_server, 0)
        self.log.debug("Total connections randomly selected for server %s: %s" % (selected_server, total_connections))
