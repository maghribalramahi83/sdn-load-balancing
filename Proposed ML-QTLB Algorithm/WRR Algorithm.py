...
class iplb(object):
    ...
    def _pick_server(self, key, inport):
        ...
        # Define a dictionary to store the total connections for each server
        total_connections_wrr = {}
        
         #range of 0 to 3 (inclusive) by taking the remainder after division by 4
        if self.weight < 2:# weight=2   (0 and 1)
            for x in self.total_connection:
                if str(x) == '10.0.0.1':
                    ipserver = x
                    self.log.debug("Best available Server: %s" % ipserver)
                    self.log.debug("Total connections WRR selected for server %s: %s" % (ipserver, totalconns))
                    self.weight += 1
                    total_connections_wrr[ipserver] = total_connections_wrr.get(ipserver, 0) + 1
                    return ipserver
        elif self.weight == 2:   # weight=1   (2)
            for x in self.total_connection:
                if str(x) == '10.0.0.2':
                    ipserver = x
                    self.log.debug("Best available Server: %s" % ipserver)
        ...
        # Write average total connections WRR selected for each server
        for server, total_connections in total_connections_wrr.items():
            average_connections = total_connections / float(len(self.total_connection))
            self.log.debug("Average total connections WRR selected for server %s: %s" % (server, average_connections))      
