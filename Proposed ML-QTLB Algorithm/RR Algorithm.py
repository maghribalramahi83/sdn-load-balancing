...
class iplb (object):
  ...
  def _pick_server (self, key, inport):
    ...
    """
    server_list = list(self.live_servers.keys())
    self.last_server = (self.last_server + 1) % len(server_list)
    selected_server = server_list[self.last_server]
    total_connections = self.total_connection.get(selected_server, 0)  # Get the current connection count
    total_connections += 1  # Increment the connection count for the selected server
    self.total_connection[selected_server] = total_connections  # Update the connection count in the dictionary
    self.log.debug("Total connections using round-robin for server %s: %s" % (selected_server, total_connections))
