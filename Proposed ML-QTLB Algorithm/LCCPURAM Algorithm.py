...
class iplb(object):
    ...
    def __init__(self, connection, service_ip, servers=[]):
        ...
        self.cpu_usages = {}
        self.memory_usages = {}  # IP -> Memory usage
        self._start_cpu_monitoring() # Start monitoring CPU usage in a separate thread
    ...
    def _start_cpu_monitoring(self):
        # Define a function that will run in a separate thread to monitor CPU usage
        def monitor_cpu_usage():
            while True:
                for server in self.servers:
                    cpu_usage = psutil.cpu_percent(interval=1.0)  # Get CPU usage for the server
                    memory_usage = psutil.virtual_memory().percent  # Get memory usage for the server
                    self.cpu_usages[server] = cpu_usage
                    self.memory_usages[server] = memory_usage

        # Start the monitoring thread
        import threading
        monitoring_thread = threading.Thread(target=monitor_cpu_usage)
        monitoring_thread.daemon = True  # Set the thread as a daemon so it stops when the main thread ends
    ...
    def _pick_server(self, key, inport):
        """
        Select server with lowest CPU usage and Memory
        """
        if len(self.live_servers) == 0:
            return None

        best_server = None
        lowest_combined_usage = float('inf')  # store the current lowest combined usage (initialized with positive infinity)

        for ipserver in self.live_servers.keys():
            cpu_usage = self.cpu_usages.get(ipserver, 0)
            memory_usage = self.memory_usages.get(ipserver, 0)
            total_connections = self.total_connection.get(ipserver, 0)
            self.log.debug("CPU usage for server %s: %s" % (ipserver, cpu_usage))
            self.log.debug("Memory usage for server %s: %s" % (ipserver, memory_usage))
            self.log.debug("total connections for server %s: %s" % (ipserver, total_connections))

            # Weighted combination of CPU and memory usage
            combined_usage = 0.6 * cpu_usage + 0.4 * memory_usage + total_connections * 0.2

            if combined_usage < lowest_combined_usage:
                best_server = ipserver
                lowest_combined_usage = combined_usage

        self.log.debug("Best available server: %s" % best_server)
        return best_server
