...
- M/M/1-based server selection using \( \lambda_k \), \( \mu_k \), \( \rho_k \), and \( W_k \); overloaded servers (\( \rho_k \ge 1 \)) are excluded by setting \( W_k = \infty \)
...
1. **Traffic Generation & Dataset Construction**
...
5. **QT Scheduling with Aging**
   ...
   M/M/1-based selection of the minimum \( W_k \), with aging-based promotion of long-waiting low-priority flows.

### Real-Time Decision Logic (per request)

1. Extract the 8 features from the incoming request.
2. Classify with RF → Class 1 (low priority) or Class 2 (high priority).
3. Compute \( \lambda_k, \mu_k, \rho_k, W_k \) for all servers from live POX counters.
4. Promote any pending Class-1 request whose accumulated waiting time ≥ T = 3.0 s.
5. Select \( S^* = \arg\min W_k \) among eligible servers, prioritizing promoted requests.
6. Install/update the flow rule in OVS via OpenFlow to route the request to \( S^* \).
...
\[
PI\% = \frac{Baseline - Proposed}{Baseline} \times 100
\]

For metrics where higher is better (Degree of LB):

\[
PI\% = \frac{Proposed - Baseline}{Baseline} \times 100
\]

Overall mean improvement across N load levels:

\[
\overline{PI\%} = \frac{1}{N}\sum_{i=1}^{N} PI\%(L_i)
\]
...
For each server \( S_k \), \( k \in \{1, 2, 3\} \), the controller computes online from POX counters:

- Arrival rate: \( \lambda_k = \dfrac{\text{arrived requests}}{\text{elapsed time}} \) (requests/s)
- Service rate: \( \mu_k = \dfrac{\text{completed requests}}{\text{elapsed time}} \) (requests/s)
- Utilization: \( \rho_k = \dfrac{\lambda_k}{\mu_k} \) (dimensionless, stability requires \( \rho_k < 1 \))
- Expected system time: \( W_k = \dfrac{1/\mu_k}{1 - \rho_k} \), for \( \rho_k < 1 \); otherwise \( W_k = \infty \)
...
Starvation prevention: a low-priority request \( r \) is promoted to high priority when its accumulated waiting time satisfies \( W_r \ge T \), with \( T = 3.0 \) s. Note that \( \lambda_k \) and \( \mu_k \) are measured live at the controller and are independent of the Wireshark dataset and the FCM/RF pipeline, which are used only for traffic-class identification.

---

## ⚠️ Limitations

1. **Single-switch topology** — three clients and three servers; results may differ in multi-switch, multi-controller deployments.
2. **M/M/1 approximation** — Poisson arrivals and exponential service times are assumed; \( W_k \) is used as a relative ranking criterion rather than an exact predictor.
3. **Fixed aging threshold** — T is set empirically; the optimal value may vary with traffic pattern and topology.
4. **Binary traffic classification** — only light/heavy classes; finer granularity may be needed in production.
5. **Static classifier** — the RF model is trained offline and does not adapt to concept drift.
